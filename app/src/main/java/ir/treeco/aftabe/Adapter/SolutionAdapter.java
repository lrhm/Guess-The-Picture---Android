...
public class SolutionAdapter extends RecyclerView.Adapter<SolutionAdapter.ViewHolder> {
    char[] characters;
    char[] status;
    GameFragment gameFragment;  //todo callback, on item click listner
    int n;
    int break0;
    int break1;
    int solutionSize;
    ...
    public class ViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        public void onClick(View v) {
            if (status[getLocalPosition(getAdapterPosition())] != '*') {
                gameFragment.removeFromSolution(getLocalPosition(getAdapterPosition()), 0);
            }
        }
    }
    ...
    public SolutionAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_solution, parent, false);
    ...
    public void onBindViewHolder(SolutionAdapter.ViewHolder holder, int position) {
        if (status[getLocalPosition(position)] == '-') {
            ...
            holder.textView.setBackgroundDrawable(gameFragment.getResources().getDrawable(R.drawable.place_holder));
            ...
            holder.itemView.setLayoutParams(new LinearLayout.LayoutParams(solutionSize, solutionSize));
            holder.textView.setBackgroundDrawable(gameFragment.getResources().getDrawable(R.drawable.albutton));
